...
namespace RenderCore { namespace Techniques 
{
	...
	void ManualDrawableGeoConstructor::FulfillWhenNotPending(std::promise<Promise>&& promise)
	{
		...
		if (_pimpl->_vb->_uploadTotal != 0) {
			...
			_pimpl->_vb->_desc = CreateDesc(BindFlag::VertexBuffer, 0, GPUAccess::Read, LinearBufferDesc::Create(vbSize), "[vb]");
		...
		if (_pimpl->_ib->_uploadTotal != 0) {
			...
			_pimpl->_ib->_desc = CreateDesc(BindFlag::IndexBuffer, 0, GPUAccess::Read, LinearBufferDesc::Create(ibSize), "[ib]");
