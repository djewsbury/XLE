	...
	public:
		void    Render(IOverlayContext& context, Layout& layout, Interactables&interactables, InterfaceState& interfaceState)
		{
			...
			if (_activeMonitorId < monitors.size()) {
				...
				DrawText().Draw(context, layout.AllocateFullWidth(lineHeight), StringMeldInPlace(buffer) << "Geometry X: " << geo._x << ", Y: " << geo._y);
				DrawText().Draw(context, layout.AllocateFullWidth(lineHeight), StringMeldInPlace(buffer) << "Geometry Width: " << geo._width << ", Geometry Height: " << geo._height);
			}
			...
			case MenuMode::ChangeMode:
				{
					if (input.IsPress(KeyId_Make("backspace"))) {
						...
						auto modes = _dispSettings->GetModes(_activeMonitorId);
						if (idx < modes.size()) {
							bool success = _dispSettings->TryChangeMode(_activeMonitorId, modes[idx]);
							if (success) {
								auto i = std::find(_monitorsReleasableMode.begin(), _monitorsReleasableMode.end(), _activeMonitorId);
								if (i == _monitorsReleasableMode.end())
