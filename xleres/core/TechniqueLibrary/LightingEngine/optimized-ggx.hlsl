...
float3 LightingFuncGGX_OPT5_XLE_Metallic(float3 N, float3 V, float3 L, float roughness, float3 F0)
{
	float3 H = normalize(V+L);
	...
	float3 FV = F0*FV_helper.x + FV_helper.y;
	float3 specular = dotNL * D * FV;
