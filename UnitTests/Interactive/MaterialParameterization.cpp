	...
	public:
			...
			IInteractiveTestHelper& testHelper) override
		{
			if (evnt._wheelDelta) {
				...
				float xRatio = 1.f - std::clamp((evnt._mousePosition[0] - context._view._viewMins[0]) / float(context._view._viewMaxs[0] - context._view._viewMins[0]), 0.f, 1.f);
				float yRatio = 1.f - std::clamp((evnt._mousePosition[1] - context._view._viewMins[1]) / float(context._view._viewMaxs[1] - context._view._viewMins[1]), 0.f, 1.f);
