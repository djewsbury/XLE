...
namespace UnitTests
{
	...
	static uint64_t Encode(Int2 q) { return (uint64_t(uint32_t(q[0])) << 32ull) | uint64_t(uint32_t(q[1])); }
	static Int2 Decode(uint64_t q) { return {q>>32ull, int32_t(q)}; }
	static unsigned RevAdj(unsigned i)
	{
		return 5-i;
		/*switch (i) {
		case 0: return 5;		// top left -> bottom right
		case 1: return 4;		// top right -> bottom left
		case 2: return 3;		// left -> right
		case 3: return 2;		// right -> left
		case 4: return 1;		// bottom left -> top right
		case 5: return 0;		// bottom right -> top left
		}
	...
	{
		std::vector<unsigned> _adjacencies;
		AdjacencyDatabase(IteratorRange<const Int2*> enabledCells)
		{
			// For given arbitrary list of enabled cells, figure out 
			std::vector<std::pair<uint64_t, unsigned>> sortedList;
			sortedList.reserve(enabledCells.size());
			for (unsigned c=0; c<enabledCells.size(); ++c) sortedList.emplace_back(Encode(enabledCells[c]), c);
			std::sort(sortedList.begin(), sortedList.end(), CompareFirst2{});

			Int2 adj[6];
			_adjacencies.resize(enabledCells.size()*6, ~0u);
			for (auto i=sortedList.begin(); i!=sortedList.end(); ++i) {
				GetAdjacentCells(adj, Decode(i->first));
				for (unsigned c=0; c<6; ++c) {
					auto e = Encode(adj[c]);
					if (e < i->first) continue;
					auto i2 = LowerBound2(MakeIteratorRange(i+1, sortedList.end()), e);
					if (i2 == sortedList.end() || i2->first != e) continue;

					// make the adjacency, both ways
					_adjacencies[i->second*6+c] = i2->second;
					_adjacencies[i2->second*6+RevAdj(c)] = i->second;
				}
			}
		}
	};

	static std::vector<Float2> CreateBoundaryLines(const HexCellField& cellField, IteratorRange<const Int2*> boundaryCellsList)
	{
		assert(!boundaryCellsList.empty());
		std::vector<Float2> boundaryLines;
		boundaryLines.reserve(boundaryCellsList.size() * 2 * 6);

		// Super primitive; but.. for each boundary cell, check which neighbours are enabled and draw
		// a line between them
		for (auto cell:boundaryCellsList) {
			Int2 adjacent[6];
	...
	template<typename Primitive>
		static Primitive SignedPolyArea(IteratorRange<const Vector2T<Primitive>*> loop)
	{
		Primitive result = 0;
		for (unsigned c=0; c<loop.size(); ++c) {
			auto next = loop[(c+1)%loop.size()];
			result += (next[0] - loop[c][0]) * (next[1] + loop[c][1]);
		}
		return result / 2.f;
	}
	...
	{
		// Taking the result._enabledCells, find all of the boundary groups
		// result._enabledCells should be one contiguous "island"
		AdjacencyDatabase adjacencies{enabledCellList};
		std::vector<Float2> boundaryLineList;
		boundaryLineList.reserve(enabledCellList.size()*6*2/2);

		for (unsigned c=0; c<enabledCellList.size(); ++c) {
			auto cell = enabledCellList[c];
			for (unsigned a=0; a<6; ++a) {
				if (adjacencies._adjacencies[c*6+a] != ~0u) continue;
				Float2 cellCenter { s_2cos30 * (float)cell[0], 1.5f * (float)cell[1] };
				if (cell[1] & 1) {
					// odd
					boundaryLineList.push_back(s_hexCornersOdds[s_hexEdges[a].first] + cellCenter);
					boundaryLineList.push_back(s_hexCornersOdds[s_hexEdges[a].second] + cellCenter);
				} else {
					// even
					boundaryLineList.push_back(s_hexCornersEvens[s_hexEdges[a].first] + cellCenter);
					boundaryLineList.push_back(s_hexCornersEvens[s_hexEdges[a].second] + cellCenter);
				}
			}
		}

		// divide the boundary lines up into loops

		using Primitive = float;
		struct PendingLoop
		{
			std::vector<Vector2T<Primitive>> _boundary; 
			unsigned _containingLoop = ~0u;
		};
		std::vector<PendingLoop> pendingLoops;

		while (!boundaryLineList.empty()) {
			PendingLoop loop;
			loop._boundary.reserve(1+boundaryLineList.size()/2);
			loop._boundary.push_back(*(boundaryLineList.end()-2));
			loop._boundary.push_back(*(boundaryLineList.end()-1));
			boundaryLineList.erase(boundaryLineList.end()-2, boundaryLineList.end());

			while (!boundaryLineList.empty()) {
				auto i = boundaryLineList.begin();
				for (;i!=boundaryLineList.end(); i+=2)
					if (Equivalent(*i, loop._boundary.back(), 1e-3f))
						break;
				assert(i != boundaryLineList.end());
				if (Equivalent(*(i+1), loop._boundary[0], 1e-3f)) {
					// reached the end of the loop
					boundaryLineList.erase(i, i+2);
					break;
				} else {
					loop._boundary.push_back(*(i+1));
					boundaryLineList.erase(i, i+2);
				}
			}

			// This is a completed loop -- like figure out it's winding
			// we can use a simple area calculation for this -- see (eg)
			//		https://www.element84.com/blog/determining-the-winding-of-a-polygon-given-as-a-set-of-ordered-points#:~:text=Winding%20can%20be%20either%20clockwise,down%20at%20the%20coordinate%20plane.&text=In%20figure%201%2C%20the%20vertices,CCW%20winding%20on%20the%20right.
			auto area = SignedPolyArea<Primitive>(loop._boundary);
			assert(area != 0);
			if (area > 1.f)
				std::reverse(loop._boundary.begin(), loop._boundary.end());		// clockwise, so reverse direction
			pendingLoops.emplace_back(std::move(loop));
		}

		// find loops that are contained within other loops. We're assuming no intersections and no
		// sharing of vertices. So we can do pt-in-polygon tests on one arbitrary point in each polygon
		// vs the other polygons
		unsigned externalPolygonIdx = ~0u;
		for (auto i=pendingLoops.begin(); i!=pendingLoops.end(); ++i) {
			// little track to find a pt slightly off the hex cell edges -- we should still get
			// the right result if we're a little inside or a little outside of the boundary
			auto testPt = i->_boundary[0];
			for (auto c=i->_boundary.begin()+1; c!=i->_boundary.end(); ++c)
				if ((*c)[1] != testPt[1]) {
					testPt += (*c - testPt) / 4;
					break;
				}

			auto i2=pendingLoops.begin();
			for (; i2!=pendingLoops.end(); ++i2)
				if (i2 != i && PtInPolygon<Primitive>(i2->_boundary, testPt))
					break;
			...
			if (i2!=pendingLoops.end()) {
				i->_containingLoop = (unsigned)std::distance(pendingLoops.begin(), i2);
					...
					for (++i2; i2!=pendingLoops.end(); ++i2)
						if (i2 != i)
							assert(!PtInPolygon<Primitive>(i2->_boundary, testPt));
				#endif
			} else {
		...
		// we should end up with exactly one external polygon
		assert(externalPolygonIdx != ~0u);

		HexCellField result;
		result._enabledCells = std::move(enabledCellList);
		result._exteriorGroup = HexCellField::BoundaryGroup{std::move(pendingLoops[externalPolygonIdx]._boundary)};
		for (unsigned c=0; c<pendingLoops.size(); ++c) {
			if (c == externalPolygonIdx) continue;
			assert(pendingLoops[c]._containingLoop == externalPolygonIdx);
			// reverse winding because the polygon bounds a void, not a solid area
			std::reverse(pendingLoops[c]._boundary.begin(), pendingLoops[c]._boundary.end());
			result._interiorGroups.push_back(HexCellField::BoundaryGroup{std::move(pendingLoops[c]._boundary)});
		}
		return result;
	}
	...
	{
		assert(cellCount > 0);
		std::vector<Int2> enabledCellList;
		...
		enabledCellList.push_back(Int2(0, 0));
		Int2 adjacent[6];
		...
		while (enabledCellList.size() < cellCount) {
			...
			assert(std::find(enabledCellList.begin(), enabledCellList.end(), *i) == enabledCellList.end());
			GetAdjacentCells(adjacent, *i);
			enabledCellList.push_back(*i);
			workingBoundaryCells.erase(i);

			for (auto c:adjacent) {
				if (std::find(enabledCellList.begin(), enabledCellList.end(), c) != enabledCellList.end()) continue;
		...
		return CreateHexField(std::move(enabledCellList));
	}
	...
	HexCellField CreateRegularHexField(unsigned radius)
	{
		...
		enabledCells.push_back(Int2(0, 0));
		Int2 adjacent[6];
		GetAdjacentCells(adjacent, Int2(0, 0));
		...
		for (unsigned r=1; r<radius; ++r) {
			...
			for (auto cell:nextBoundaryCells) {
				assert(std::find(enabledCells.begin(), enabledCells.end(), cell) == enabledCells.end());
				...
				for (auto a:adjacent) {
					...
					i = std::find(enabledCells.begin(), enabledCells.end(), a);
					if (i != enabledCells.end()) continue;
	...
	std::vector<std::pair<HexCellField, Int2>> CreateFromMultipleCellIslands(IteratorRange<const Int2*> enabledCells)
	{
		...
		searchQueue.reserve(workingEnabledCells.size());
		while (!workingEnabledCells.empty()) {
			// find the network of connected cells
			std::vector<Int2> island;
			island.reserve(workingEnabledCells.size());
			Int2 original = workingEnabledCells.back();
			workingEnabledCells.pop_back();
			island.push_back(original);

			searchQueue.clear();
			searchQueue.push_back(original);
			while (!searchQueue.empty()) {
				auto search = searchQueue.back();
				searchQueue.pop_back();
				Int2 adjacent[6];
				GetAdjacentCells(adjacent, search);
				for (auto a:adjacent) {
					auto i = std::find(workingEnabledCells.begin(), workingEnabledCells.end(), a);
					if (i != workingEnabledCells.end()) {
						searchQueue.push_back(*i);
						island.push_back(*i);
						workingEnabledCells.erase(i);
					}
				}
			}
	...
	{
		std::vector<Int2> enabledCells {
			Int2{-4, 27}, Int2{-4, 26}, Int2{-3, 26}, Int2{-2, 26}, Int2{-4, 25}, Int2{-3, 25}, Int2{-2, 25}, Int2{-3, 24}, Int2{-2, 24}, Int2{-1, 24}, Int2{-3, 23}, Int2{-2, 23}, Int2{-1, 23}, Int2{-2, 22}, Int2{-1, 22}, Int2{0, 22}, Int2{-2, 21}, Int2{-1, 21}, Int2{0, 21}, Int2{0, 20}, Int2{1, 20}, Int2{1, 21}, Int2{2, 20}, Int2{1, 19}, Int2{2, 19}, Int2{3, 20}, Int2{3, 19}, Int2{3, 18}, Int2{4, 18}, Int2{3, 17}, Int2{4, 17}, Int2{5, 18}, Int2{5, 17}, Int2{5, 16}, Int2{6, 16}, Int2{4, 15}, Int2{5, 15}, Int2{6, 15}, Int2{6, 14}, Int2{7, 14}, Int2{6, 13}, Int2{7, 13}, Int2{8, 14}, Int2{8, 13}, Int
		};
		return CreateHexField(std::move(enabledCells));
	}
	...
	{
		return Truncate(localToWorld * Float3{pt, 1.f});
	}

	void DrawBoundary(RenderOverlays::IOverlayContext& overlayContext, const HexCellField& cellField, const HexCellField::BoundaryGroup& group, const Float3x3& localToWorld, RenderOverlays::ColorB color)
	{
		assert(!group._boundaryLineLoop.empty());
		std::vector<Float3> boundaryLines;
		boundaryLines.reserve(group._boundaryLineLoop.size()*2);
		auto lastPt = group._boundaryLineLoop.back();
		for (auto pt:group._boundaryLineLoop) {
			boundaryLines.push_back(Float3(TransformPoint(localToWorld, lastPt), 0.f));
	...
	void FillHexGrid(RenderOverlays::IOverlayContext& overlayContext, IteratorRange<const Int2*> enabledCells, const Float3x3& localToWorld, RenderOverlays::ColorB color)
	{
		...
		triangles.reserve(enabledCells.size()*dimof(indices));

		for (auto cell:enabledCells) {
			Float2 cellCenter { s_2cos30 * (float)cell[0], 1.5f * (float)cell[1] };
			auto* corners = (cell[1] & 1) ? s_hexCornersOdds : s_hexCornersEvens;
			for (auto t:indices)
		...
		IteratorRange<const typename StraightSkeleton<Primitive>::Edge*> edgesInit)
	{
		...
		while (!edges.empty()) { 
			...
			while (searchingVertex != endVtx) {
				...
				// If this vertex is already in the working loop, it means we've just closed a subloop
				auto existing = std::find(workingLoop.begin(), workingLoop.end(), searchingVertex);
				if (existing != workingLoop.end()) {
					result.emplace_back(existing, workingLoop.end());
					workingLoop.erase(existing, workingLoop.end());
				}
			}
	...
	public:
		...
		void Draw(RenderOverlays::IOverlayContext& overlayContext, const Float4x4& localToWorld, RenderOverlays::ProjectionMode projectionMode) const
		{
			...
			for (auto& b:_orderedBoundaryPts)
				for (unsigned c=0; c<b.size(); ++c)
					overlayContext.DrawQuad(projectionMode, Float3{b[c] - Float2{vertexSize, vertexSize}, 0.f}, Float3{b[c] + Float2{vertexSize, vertexSize}, 0.f}, RenderOverlays::ColorB(0x7f, c>>8, c&0xff));
		...
		StraightSkeletonPreview(const HexCellField& cellField, Primitive maxInset = std::numeric_limits<Primitive>::max())
		{
			...
			{
				calculator.AddLoop(MakeIteratorRange(cellField._exteriorGroup._boundaryLineLoop));
				_orderedBoundaryPts.push_back(cellField._exteriorGroup._boundaryLineLoop);
			}
			for (auto& group:cellField._interiorGroups) {
				calculator.AddLoop(MakeIteratorRange(group._boundaryLineLoop));
				_orderedBoundaryPts.push_back(group._boundaryLineLoop);
			}
	...
	static RenderCore::Techniques::CameraDesc StartingCamera(Float2 mins, Float2 maxs)
	{
		RenderCore::Techniques::CameraDesc visCamera;
		visCamera._cameraToWorld = MakeCameraToWorld(Normalize(Float3{0.f, 0.0f, -1.0f}), Normalize(Float3{0.0f, 1.0f, 0.0f}), Float3{0, 0, 200.0f});
		visCamera._projection = RenderCore::Techniques::CameraDesc::Projection::Orthogonal;
		visCamera._nearClip = 0.f;
			Float2{5.56679678,6.717968941},
			Float2{5.468381882,6.687838554},
			Float2{5.416666031,6.672005653},
			Float2{5.266536713,6.717968941},
			Float2{5.149349213,6.81601572},
			Float2{5.090974331,6.886201859},
			Float2{5.054752827,6.929752827},
			Float2{5.002669334,7.086002827},
			Float2{4.996549606,7.288215637},
			Float2{5.003239632,7.496876717},
			Float2{4.997330666,7.705664635},
			Float2{4.945247173,7.861914635},
			Float2{4.874191284,7.947347641},
			Float2{4.726910591,8.068754196},
			Float2{4.583333015,8.125},
			Float2{4.43320322,8.183202744},
			Float2{4.31601572,8.274349213},
			Float2{4.274163723,8.324668884},
			Float2{4.221419334,8.388085365},
			Float2{4.172786713,8.547786713},
			Float2{4.154522896,8.761950493},
			Float2{4.108463287,8.900129318},
			Float2{4.016536713,9.016536713},
			Float2{3.961002827,9.169335365},
			Float2{3.958605766,9.354009628},
			Float2{3.958635092,9.455756187},
			Float2{3.959114552,9.584114075},
			Float2{3.897460938,9.737695694},
			Float2{3.753450394,9.789779663},
			Float2{3.599869728,9.733463287},
			Float2{3.486132622,9.644205093},
			Float2{3.409958363,9.644205093},
			Float2{3.275130272,9.733463287},
			Float2{3.125,9.786328316},
			Float2{2.974869728,9.733463287},
			Float2{2.874511003,9.654704094}
		};
...

		{
			...
			tester->_inputs.emplace_back(std::vector<Float2>(difficultSquigglyLoop, &difficultSquigglyLoop[dimof(difficultSquigglyLoop)]));
	...
	T1(Primitive) static void DrawPrimitive(
		...
		Float2 cameraMins, Float2 cameraMaxs,
	...
	TEST_CASE( "StraightSkeletonDrawImages", "[math]" )
	{
		...
		DrawPrimitive<float>(*testHelper, fbHelper, StraightSkeletonPreview<float>{MakeIteratorRange(rectangleCollapse), 2.0f}, {-25, -25}, {25, 25}, "ss-rectangle-0");
		DrawPrimitive<float>(*testHelper, fbHelper, StraightSkeletonPreview<float>{MakeIteratorRange(rectangleCollapse), 7.0f}, {-25, -25}, {25, 25}, "ss-rectangle-1");
		DrawPrimitive<float>(*testHelper, fbHelper, StraightSkeletonPreview<float>{MakeIteratorRange(rectangleCollapse), 20.0f}, {-25, -25}, {25, 25}, "ss-rectangle-2");

		DrawPrimitive<float>(*testHelper, fbHelper, StraightSkeletonPreview<float>{MakeIteratorRange(LShapeCollapse), 3.5f}, {-25, -25}, {25, 25}, "ss-lshape-0");
		DrawPrimitive<float>(*testHelper, fbHelper, StraightSkeletonPreview<float>{MakeIteratorRange(LShapeCollapse), 5.0f}, {-25, -25}, {25, 25}, "ss-lshape-1");
		DrawPrimitive<float>(*testHelper, fbHelper, StraightSkeletonPreview<float>{MakeIteratorRange(LShapeCollapse), 20.0f}, {-25, -25}, {25, 25}, "ss-lshape-2");

		DrawPrimitive<float>(*testHelper, fbHelper, StraightSkeletonPreview<float>{MakeIteratorRange(singleMotorcycle), 3.5f}, {-25, -25}, {25, 25}, "ss-singlemotor-0");
		DrawPrimitive<float>(*testHelper, fbHelper, StraightSkeletonPreview<float>{MakeIteratorRange(singleMotorcycle), 5.0f}, {-25, -25}, {25, 25}, "ss-singlemotor-1");
		DrawPrimitive<float>(*testHelper, fbHelper, StraightSkeletonPreview<float>{MakeIteratorRange(singleMotorcycle), 20.0f}, {-25, -25}, {25, 25}, "ss-singlemotor-2");

		std::mt19937_64 rng(815283149);
		auto cellField = CreateRandomHexCellField(32, rng);
		DrawPrimitive<float>(*testHelper, fbHelper, StraightSkeletonPreview<float>{cellField, 0.5f}, {-12.5, -12.5+3}, {12.5, 12.5+3}, "ss-hex-0");
		DrawPrimitive<float>(*testHelper, fbHelper, StraightSkeletonPreview<float>{cellField, 1.0f}, {-12.5, -12.5+3}, {12.5, 12.5+3}, "ss-hex-1");
		DrawPrimitive<float>(*testHelper, fbHelper, StraightSkeletonPreview<float>{cellField, 10.0f}, {-12.5, -12.5+3}, {12.5, 12.5+3}, "ss-hex-2");

		rng = std::mt19937_64(6385264);
		auto largeCellField = CreateRandomHexCellField(96, rng);
		DrawPrimitive<float>(*testHelper, fbHelper, StraightSkeletonPreview<float>{largeCellField, 0.3f}, {-16.5-1, -16.5-1}, {16.5-1, 16.5-1}, "ss-hex-large-0");
		DrawPrimitive<float>(*testHelper, fbHelper, StraightSkeletonPreview<float>{largeCellField, 0.5f}, {-16.5-1, -16.5-1}, {16.5-1, 16.5-1}, "ss-hex-large-1");
		DrawPrimitive<float>(*testHelper, fbHelper, StraightSkeletonPreview<float>