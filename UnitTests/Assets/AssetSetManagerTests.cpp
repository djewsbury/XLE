...
namespace UnitTests
{
	...
	TEST_CASE( "Assets-ConstructToPromise", "[assets]" )
	{
		...
		auto futureAsset = ::Assets::GetAssetMarker<ExampleAsset>("ut-data/file.dat");
		...
		auto failedAsset = ::Assets::GetAssetMarker<ExampleAsset>("ut-data/no-file.data");
		...
		REQUIRE(!log.empty());
		Log(Debug) << "Failed GetAssetFuture<> reported: " << log << std::endl;
	}
