...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    , _pool(std::move(pool))
    {
        ...
        if (!initialReferenceAlreadyTaken) {
            ...
            if (strongPool && _resource)
                strongPool->AddRef(*_resource, _interiorOffset, _interiorSize);
        }
    }
    ...
    ResourceLocator::~ResourceLocator() 
    {
        ...
        if (pool)
            pool->Release(*_resource, _interiorOffset, _interiorSize);
    }
    ...
    , _completionCommandList(copyFrom._completionCommandList)
    {
        if (_managedByPool) {
            auto pool = _pool.lock();
            if (pool && _resource)
                pool->AddRef(*_resource, _interiorOffset, _interiorSize);
        }
    }
