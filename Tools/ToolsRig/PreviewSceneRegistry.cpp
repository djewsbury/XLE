...
namespace ToolsRig
{
	...
	public:
		...
		::Assets::PtrToMarkerPtr<SceneEngine::IScene> CreateScene(StringSection<> sceneName, const std::shared_ptr<RenderCore::Techniques::IDrawablesPool>& drawablesPool, const std::shared_ptr<RenderCore::Techniques::IPipelineAcceleratorPool>& pipelineAccelerators, const std::shared_ptr<RenderCore::Techniques::IDeformAcceleratorPool>& deformAccelerators, const std::shared_ptr<::Assets::OperationContext>& loadingContext)
		{
			for (auto i=_registrySet.begin(); i!=_registrySet.end(); ++i) {
